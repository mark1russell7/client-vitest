...
const code = `import { createProcedure, registerProcedures, type Procedure } from "@mark1russell7/client";
import { z } from "zod";
import {
...
const vitestRunProcedure: Procedure = createProcedure()
  .path(["vitest", "run"])
...
const vitestWatchProcedure: Procedure = createProcedure()
  .path(["vitest", "watch"])
...
fs.writeFileSync(path.join(__dirname, 'src/register.ts'), code);
console.log('Written register.ts');
